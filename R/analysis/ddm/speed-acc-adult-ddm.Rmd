...
Make a table of number of "good" shifts for each condition. 
...
sub.pars.df <- d.ddm %>% 
  group_by(subid, condition) %>% 
  do(fit_ddm(., condition_col = "condition", subid_col = "subid", bysub = T)) 
```
...
sub.pars.df %<>% 
  ...
  filter(convergence == 0, fit.vals > 0) %>% 
...
ggplot(aes(x = condition, y = fit.vals, fill = condition), data = sub.pars.df) +
  geom_boxplot() +
  guides(fill = F) +
