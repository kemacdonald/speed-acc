...
d.adults <- files.adults %>% 
  purrr::map_df(read_hddm_file, path = hddm.adults.path, 
                condition_list = c("bull", "face", "text_no_audio", "text",
                                   "straight_ahead_clear", "straight_ahead_noise", "gaze_clear", "gaze_noise"), 
...
d.adults %<>% 
  mutate(age_code = "adult", 
         experiment = case_when(
           ...
           condition %in% c("straight_ahead_clear", "straight_ahead_noise", "gaze_clear", "gaze_noise") ~ "noise_gaze",
           TRUE ~ "NA"
         )
  )
...
```{r}
boundary_asl <- d %>% filter(param_name == "boundary", condition == "asl", age_code == "kid")
boundary_face <- d %>% filter(param_name == "boundary", condition == "face", age_code == "kid")
p_hddm_boundary <- mean(boundary_face$param_value > boundary_asl$param_value)

# drift
drift_asl <- d %>% filter(param_name == "drift", condition == "asl", age_code == "kid")
drift_face <- d %>% filter(param_name == "drift", condition == "face", age_code == "kid")
