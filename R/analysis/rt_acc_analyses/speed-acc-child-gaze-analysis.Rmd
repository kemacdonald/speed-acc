...
ss %>% 
  ...
  geom_jitter(width = 0.05, alpha = 0.6, aes(color = as.factor(age_group))) + 
  geom_hline(yintercept = 0.5, linetype = "dashed") #+
  #geom_pointrange(data = ms, aes(ymin = summary_ci_lower, ymax = summary_ci_upper), color = "darkgreen", size = 1)
```
...
p <- d.filt %>% 
  filter(is.na(rt) == F,
         shift_type %in% c("C_T", "C_D")) %>% 
  ggplot(aes(x = forcats::fct_rev(gaze_condition), y = rt, color = shift_accuracy, 
             fill = shift_accuracy)) +
