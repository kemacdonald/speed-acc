...
```{r}
table_adult <- read_csv("speed_acc_adult_ng_fstshift_tidy.csv")
```
...
#gaze_trials <- table(table1["gaze_condition"])
#num_no_gaze <- gaze_trials["no_gaze"]
#num_gaze <- gaze_trials["gaze"]
```
...
```{r}
table_adult.analysis %>% 
  ggplot() + geom_bar(mapping = aes(x = rt, fill = noise_condition)) +
