...
```{r}
#getwd()
#setwd("~/Desktop/Research/speed-acc/data/3_final_merged_data/first_shifts")
#d <- read_csv("speed_acc_adult_ng_fstshift_tidy.csv")
...
Check out what's going on with #8

```{r}
tmp <- d %>% filter(subid == "speed_acc_adult_ng_8")
```
...
```{r}
#data.frame(d)
ggplot(d) + geom_point(mapping = aes(x = subid, y = rt))
```
...
```{r}
   ggplot(d) + geom_bar(mapping = aes(x = subid, fill = shift_accuracy)) +
...
library(ggplot2)
data.frame(d)
ggplot(data.frame(d)) + geom_bar(mapping = aes(x = shift_accuracy))
```
...
library(ggplot2)
data.frame(d)
ggplot(data.frame(d)) + geom_bar(mapping = aes(x = gaze_condition, fill = shift_accuracy))

library(ggplot2)
data.frame(d)
ggplot(data.frame(d)) + geom_bar(mapping = aes(x = noise_condition, fill = shift_accuracy))
```
...
# aggreate at the condition level
ms <- ss %>% 
  filter(shift_accuracy == "correct") %>% 
  group_by(condition_long) %>% 
  summarise(mean_correct = mean(prop),
            sd_correct = sd(prop))
```
...
**KM note**: next, I would try to make a similar boxplot for the average RT for each condition. Remember that we are now storing the information about condition in the `condition_long` variable.
