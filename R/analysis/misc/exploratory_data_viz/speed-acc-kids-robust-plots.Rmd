---
title: "Speed-Acc Kids Robust Graphical Methods "
...
library(here); library(rogme) # key package for making visualizations
...
```{r read data}
d <- read_csv(here::here(data_path, "speed-acc-child-trio-fst-shift.csv"))
```

Clean the spoken English and sign language datasets, so we can merge them.

```{r clean datasets to plot}
d %<>% 
...
d %<>% mutate(shift_accuracy = ifelse(shift_type == "C_T", "correct", "incorrect"))
```
...
p <- d %>% 
  filter(response_onset_type == "noun",
         is.na(rt) == F) %>% 
  ggplot(aes(x = forcats::fct_rev(condition), y = rt, 
