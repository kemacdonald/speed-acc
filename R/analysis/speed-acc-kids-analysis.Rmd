...
```{r libraries, echo = F}
library(langcog); library(magrittr); library(RWiener)
library(stringr); library(GGally); 
library(lme4); library(lsr); library(plyr)
library(gridExtra); library(dplyr); library(tidyr)
library(ggplot2)
...
Boxplot for parameter values

```{r}
sub.pars$Condition <- factor(sub.pars$Condition, 
                                levels = c("ASL", "Face", "Object", "Bullseye"))

sub.pars$language_modality <- ifelse(sub.pars$Condition == "ASL", "ASL", "English")

ggplot(aes(x = Condition, y = Value, fill = language_modality), 
       data = filter(sub.pars, Param %in% c("Drift", "Separation", "Non.Decision"))) +
...
# columns need to be named "q" for RT and "resp" for response
d.adults <- df %>% 
  filter(age_code == "adult", trial_type != "no_shift", is.na(RT_sec) == F, 
         RT_sec > .1) %>% 
...
ewma <- function(data){
  ...
  ggplot(ewma_results, aes(rt)) +
    geom_line(aes(y = cs, color = "cs"), size = 1) +
    geom_line(aes(y = ucl, color = "UCL"), size = 1) +
...
## TODO - scatterplot of RT vs dwelltime on incorrect in ASL vs spoken
```
...
Graphed:
```{r plotpars3, fig.width=8, fig.height=6}
...
Density graph:
```{r plotpars2, fig.width=8, fig.height=6}
