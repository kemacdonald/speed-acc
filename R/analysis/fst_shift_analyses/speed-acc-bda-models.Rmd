...
d_ewma_noise <- noise_ewma_files %>% purrr::map_df(read_ewma, path = ewma_path) 
```
...
m_guessing_noise <- stan_glm(
  guess_num ~ noise_condition + age_category,
  data = d_ewma_noise,
  prior = normal(0, 2),
  prior_intercept = normal(0, 1))
...
# summarise group means
ms_guess_noise <- samples_ewma_guess_noise %>% 
  group_by(age_category, noise_condition) %>% 
  summarise(MAP = mean(param_est),
            hdi_lower = quantile(param_est, probs = 0.025),
            hdi_upper = quantile(param_est, probs = 0.975)) %>% 
