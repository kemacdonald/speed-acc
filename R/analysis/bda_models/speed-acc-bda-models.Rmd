...
```{r acc noise model continous age}
m_bglm_acc_contAge_e1 <- stan_glmer(
  shift_acc_num ~ noise_condition + age + (noise_condition + target_image | subid), 
...
```{r acc noise age continuous interaction}
...
```{r rt noise model age categorical}
...
```{r rt noise age continous}
m_bglm_rt_contAge_e1 <- stan_glmer(
  log_rt ~ noise_condition + age + (noise_condition + target_image | subid), 
  family = gaussian(),
  data = filter(d_e1_analysis, age_category == "children"),
  prior = normal(0, 2), 
  prior_intercept = normal(0, 5),
...
```{r gaze model extract posterior samples, eval = F}
...
samples_ewma_guess_noise <- m_guessing_noise %>% 
  ...
  dplyr::rename(clear_adults = `(Intercept)`,
                noise_beta = noise_conditionnoise,
                age_beta = age_categorychildren,
                int_beta = `noise_conditionnoise:age_categorychildren`) %>% 
  select(clear_adults, noise_beta, age_beta, int_beta) %>% 
  mutate(clear_children = clear_adults + age_beta,
         noise_children = clear_adults + age_beta + noise_beta*int_beta, 
         noise_adults = clear_adults + noise_beta,
         sample_id = 1:n()) %>% 
  gather(key = condition, value = param_est, -sample_id, -noise_beta, -age_beta, -int_beta) %>% 
  separate(condition, into = c("noise_condition", "age_category"))
```

Fit a model with age as continous predictor just within the child sample.

```{r prop guessing ewma age continous}
# fit model
...
posteriors <- list(rt_noise = samples_e1_rt_tidy,
                   rt_noise_age = samples_e1_acc_age_cont,
