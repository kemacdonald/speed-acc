...
d_asl <- read_csv(here::here(data_path, "speed_acc_child_trio_fstshift_tidy.csv"))
```
...
samples_text_acc <- m_acc %>% 
  ...
  dplyr::rename(text_no_audio_int = `(Intercept)`,
                face = conditionface,
                text = conditiontext,
                bullseye = conditionbullseye) %>% 
  mutate(sample_id = 1:n()) %>% 
  select(sample_id, bullseye, face, text, text_no_audio_int)
```
...
samples_text_ewma_cuts <- m_ewma_cuts %>% 
  ...
  mutate(sample_id = 1:n()) %>% 
  select(sample_id, bullseye:text_no_audio)
```
