...
```{r read data}
df <- read_csv("../../../data/3_final_merged_data/speed-acc-ss-df.csv") 
...
=======
  ggtitle("Accuracy")
>>>>>>> b14a47db7bcc49f07e5f61496118a7578884dc3f
```
...
ggplot(aes(x = stimuli, y = mean_rt / 1000, fill = as.factor(correct)), 
  ...
  theme(text = element_text(size = 16))
=======
ggplot(aes(x = stimuli, y = mean_rt, fill = as.factor(correct)), 
       data = filter(ss_prop, age_code == "child")) + 
  geom_boxplot(outlier.colour = "darkgreen") +
  geom_point(pch = 21, position = position_jitterdodge(), alpha = 0.75) +
  scale_fill_solarized() +
  labs(x = NULL, y = "Median RT (msec)", title = "Reaction Time", fill = "Correct") +
  coord_flip() 
>>>>>>> b14a47db7bcc49f07e5f61496118a7578884dc3f
```
...
ggplot(aes(x = RT, fill = Response.Type), 
  ...
  facet_grid(.~stimuli) +
  geom_vline(aes(xintercept = med_rt, color = Response.Type), size = 1, lty = "dashed", 
...
Write EWMA results to .csv file for use with the Drift Diffusion analysis. 

## Stats over EWMA parameters
