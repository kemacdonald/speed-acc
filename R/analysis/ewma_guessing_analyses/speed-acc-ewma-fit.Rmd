...
```{r}
df_gaze_kid %<>% mutate(correct = ifelse(shift_accuracy_clean == "correct", 1, 0))
```
...
```{r}
df_text_adult %<>% mutate(correct = ifelse(shift_accuracy == "correct", 1, 0))
```
...
ss.ewma.text.adult <- df_text_adult %>% 
  ...
  select(-rt) %>% 
  group_by(condition) %>% 
  do(ewma_function(., rt_column = "RT", L = 3, lambda = lambda, cs = cs, sigma = sig)) %>% 
  mutate(rt = round(as.numeric(RT), 2),
         cs = round(as.numeric(cs), 2),
         ucl = round(as.numeric(ucl), 2),
         guess = ifelse(cs < ucl, "guess", "response"),
...
```{r}
df_ng_adult %<>% mutate(correct = ifelse(shift_accuracy == "correct", 1, 0),
...
ss.ewma.ng.adult <- df_ng_adult %>% 
  filter(keep_et == "include", 
         ...
         !(is.na(rt)), 
         response_onset_type == "noun") %>% 
  dplyr::select(subid, tr.num, rt, condition_long, correct) %>% 
