...
```{r}
df_trio_kid <- read_csv(paste0(read.path, "speed-acc-child-trio-fst-shift.csv"))
df_noise_kid <- read_csv(paste0(read.path, "speed_acc_child_noise_fstshift_tidy.csv"))
df_gaze_kid <- read_csv(paste0(read.path, "speed_acc_child_gaze_fstshift_tidy.csv"))
df_ng_adult <- read_csv(paste0(read.path, "speed_acc_adult_ng_fstshift_tidy.csv"))
df_text_adult <- read_csv(paste0(read.path, "speed_acc_adult_text_fstshift_tidy.csv"))
```

Set the global EWMA model parameters. These parameters are taken from Vandekerckhove and Tuerlinckx, 2007. 
...
df_gaze_kid %<>% 
  ...
  filter(keep_et == "include", 
         ...
         !(is.na(rt)), 
         response_onset_type == "noun") %>% 
  dplyr::select(subid_short, tr.num, rt, gaze_condition, correct, age, age_group) %>% 
...
df_text_adult %<>% 
  ...
  filter(keep_et == "include", 
         ...
         !(is.na(rt)), 
         response_onset_type == "noun") %>% 
  dplyr::select(subid, tr.num, rt, condition, correct) %>% 
...
df_ng_adult %<>% 
  ...
  filter(keep_et == "include", 
         ...
         !(is.na(rt)), 
         response_onset_type == "noun") %>% 
  dplyr::select(subid_short, tr.num, rt, condition_long, correct) %>% 
...
write_csv(ss.ewma.trio, paste0(wr.path, "speed_acc_kids_trio_ewma_results.csv"))
write_csv(ss.ewma.gaze.kid, paste0(wr.path, "speed_acc_kids_gaze_ewma_results.csv"))
write_csv(ss.ewma.noise.kid, paste0(wr.path, "speed_acc_kids_noise_ewma_results.csv"))
write_csv(ss.ewma.text.adult, paste0(wr.path, "speed_acc_adult_text_ewma_results.csv"))
write_csv(ss.ewma.ng.adult, paste0(wr.path, "speed_acc_adult_ng_ewma_results.csv"))
```
