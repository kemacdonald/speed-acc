...
ggplot(aes(x = condition, y = med_rt, fill = shift_accuracy), data = ss.rt) +
  ...
  geom_point(alpha = 0.6, position = position_jitterdodge(jitter.width = 0.3, 
                                                          dodge.width = 0.5)) +
  scale_fill_manual(values = c("darkorange", "dodgerblue")) +
  labs(x = "Condition", 
       y = "Median RT (sec)") +
...
```{r}
ss.acc <- ss.d.rt %>% 
  filter(response_onset_type == "sentence") %>% 
...
ggplot(aes(x = fct_rev(condition), y = prop.guessing), 
  ...
  geom_boxplot(fill = "dodgerblue", width = 0.7, alpha = 0.7) +
