...
```{r}
d %>% select(subid, gender, run_date, comments, keep_et) %>% 
  unique() %>% 
  kable()
```
...
```{r}
d %<>% 
  mutate(noun_onset_ms = (noun_onset_sec * 1000) + (noun_onset_frames * 30),
         noun_onset_seconds = noun_onset_ms / 1000,
         sentence_onset_ms = (sentence_onset_sec * 1000) + (sentence_onset_frames * 30),
...
d.timecourse <- filter(d, response_onset_type == "noun") %>% 
  select(subid, subid_short, t.stim, gaze_condition, noise_condition, tr.num, 
         noun_onset_sec, gaze_target:shift_accuracy, target_image, keep_et)

# first shift
d.fs <- d %>% 
  select(subid, subid_short, tr.num, gaze_condition, noise_condition, 
         target_image, run_date:comments, 
         response:shift_accuracy, keep_et) %>% 
  unique()
```
