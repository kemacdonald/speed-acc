...
```{r}
d %>% select(subid, gender, run_date, keep_et, keep_runsheet) %>% 
  unique() %>% 
  kable()
```
...
d.timecourse <- d %>%
  ...
  select(subid, subid_short, x, y, t.stim, gaze_condition, noise_condition, target_image,
         ...
         shift_start_location, block, keep_runsheet, keep_et, resp_onset_type_fact)
...
d.fs <- d %>% 
  select(subid, subid_short, tr.num, gaze_condition, noise_condition, target_image,
         ...
         block, keep_runsheet, keep_et, resp_onset_type_fact) %>% 
  unique()
```
