...
d <- read_csv(here::here("data/2_cleaned_data/speed_acc_processed_child_noise.csv.gz"))
...
```{r}
trialinfo_d <- read_csv(here::here("data/0b_trial_information/speed-acc-child-noise-trial-info.csv"))
...
```{r}
subinfo_df <- readxl::read_xlsx(here::here("data/0a_demographics/speed_acc_child_noise_Runsheet_Bing.xlsx"))
...
```{r}
black_list_stimuli <- c("UE-keypress VolumeUp", "UE-keypress VolumeDown",
...
d %<>% filter(!(stimulus %in% black_list_stimuli))
```
...
```{r}
d %>% 
  distinct(subid, gender, keep_runsheet, keep_et, comments) %>% 
  filter(keep_runsheet == "exclude" | keep_et == "exclude") -> d_exclusions
```
...
Check how many trials we have for each participant:
...
Add ss with feweer than 50% trials to exclusions table and save.

```{r}
d %>% 
  distinct(subid, tr.num, gender, keep_runsheet, keep_et, comments) %>% 
  count(subid, gender, keep_runsheet, keep_et, comments, name = "n_trials") %>% 
  mutate(keep_trial_filter = ifelse(n_trials < 16, "exclude", "include")) -> d_tr_filter

# add information back to data frame
left_join(d, d_tr_filter) -> d

# save
d_tr_filter %>% 
  left_join(d_exclusions) %>% 
  filter(keep_trial_filter == "exclude" | keep_runsheet == "exclude" | keep_et == "exclude") %>% 
  write_csv(here::here(filter_path, 'speed-acc-child-noise-sub-filter.csv'))
```
...
```{r}
check <- d %>% 
  filter(subid_short == 2, tr.num == 16,
         response_onset_type == "noun") %>% 
  ungroup() %>% 
  select(t.stim, t.rel.noun,
         rt, gaze_target, 
         response, shift_type,
         shift_accuracy)
```
...
d.timecourse <- d %>%
  ...
  select(subid, subid_short, x, y, t.stim, gaze_condition, noise_condition, target_image,
         tr.num, noun_onset_sec, gaze_target:shift_accuracy, shift_accuracy_clean, 
         shift_start_location, block, keep_runsheet, keep_et, keep_trial_filter, resp_onset_type_fact)
...
d.fs <- d %>% 
  select(subid, subid_short, tr.num, gaze_condition, noise_condition, target_image,
         ...
         block, keep_runsheet, keep_et, keep_trial_filter, resp_onset_type_fact) %>% 
  unique()
```
