...
subinfo_df %<>% mutate(subid = str_to_lower(subid),
                       ...
                       subid = str_replace(subid, pattern = "_noise", "_child_noise"))
...
not sure what's going on with participant #P01?
  ...
  mutate(t.stim = ifelse(t.stim == 0, 0, round(t.stim, digits = 3)),
         ...
         target_looking = ifelse(gaze_target == "center", "center",
                                 ifelse(gaze_target == target_side, "target", 
                                        "distracter")),
         t.rel.noun = t.stim - noun_onset_sec,
...
write_csv(d.fs, path = paste0(write.path.fst, "speed_acc_child_noise_fstshift_tidy.csv.gz"))
```