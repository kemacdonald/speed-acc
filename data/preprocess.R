################################################################################
## PREPROCESSING SCRIPT FOR SPEED-ACC EXPERIMENT
## read in data files and consolidate them
##
## KM and AV 
################################################################################

## PRELIMINARIES
rm(list = ls())
library(dplyr)
library(readr)
library(magrittr)
library(stringr)
source("et_helper.R")

raw.data.path <- "1_raw_data/eye_tracker"
processed.data.path <- "3_final_merged_data/"

## LOOP TO READ IN FILES
all.data <- data.frame()
files <- dir(raw.data.path,pattern="*.txt")

for (file.name in files) {
  print(file.name)
  
  ## these are the two functions that are most meaningful
  d <- read.smi.idf(paste(raw.data.path,file.name,sep=""), header.rows=32)
  d <- preprocess.data(d) 
  d$subid <- str_sub(file.name, 0, str_locate(file.name, "-")[1] - 1) # hacky way to get the subid out of the file.name
  
  ## now here's where data get bound together
  all.data <- bind_rows(all.data, d)
}

## WRITE DATA OUT TO CSV FOR EASY ACCESS
write_csv(all.data, paste0(processed.data.path, "speed_acc_processed_adult_data.csv")) 

