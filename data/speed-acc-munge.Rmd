...
### Add variable to track language, center stimuli, and age bin within kids

```{r}
sol_df %<>% mutate(language_modality = "ASL", age_code_m = NA)
trio_face %<>% mutate(language_modality = "English", stimuli = "Face", age_code_m = "26")
trio_26 %<>% mutate(language_modality = "English", stimuli = "Trio", age_code_m = "26")
trio_36 %<>% mutate(language_modality = "English", stimuli = "Trio", age_code_m = "36")
trio_bull_adult %<>% mutate(language_modality = "English", stimuli = "Bull", age_code_m = NA)
trio_bull_26 %<>% mutate(language_modality = "English", stimuli = "Bull", age_code_m = "26")
```
...
```{r}
sol_df_final <- sol_df %>% 
    select(Sub.Num, Months, Tr.Num, stimuli, Months, age_code_m,
           Prescreen.Notes, language_modality, X0:X3000) 
...
iChart  <- left_join(iChart, include, by = c("Sub.Num", "stimuli", "Months")) %>% 
...
now we filter, removing ss that we determined not to analyze a priori 

```{r}
iChart <- iChart %>% 
    filter(include == "yes" | language_modality == "English", 
           Sub.Num != "Sub Num" | Sub.Num != "") 
```
