...
```{r}
ms <- ss_prop %>%
  filter(age_code == "child", Months <= 46) %>%
  distinct(Sub.Num) %>%
  group_by(language_modality, age_code, stimuli) %>% 
...
```{r}
m1_lm <- lm(mean_correct ~ stimuli, data = filter(ss_prop, age_code == "child", correct == 1))
...
Plot correct and incorrect RTs
...
ggplot(aes(x = RT, fill = as.factor(correct)), 
       data = filter(df, age_code == "child", stimuli == "Bunny")) +
...
## Proportion correct at each time slice
