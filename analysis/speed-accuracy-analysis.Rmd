...
# compute mean accuracy for each RT quartile and experiment
ms_time_slice_acc <- df %>% 
...
# compute mean rt for each RT quartile and experiment
ms_time_slice_rt <- df %>% 
  filter(age_code == "child", is.na(RT_sec) == F, RT <= 2000) %>% 
  mutate(RT_binned = cut(RT_sec, breaks = 4),
         RT_binned_q = lsr::quantileCut(RT_sec, 4)) %>%
...
Here we model the increase in goal-driven behavior over time in the task.
...
## todo: fix merge, so we can add the mean RT data for each bin to the mpt data frame
# left_join(mpt_data, select(ms_time_slice, mean_rt, stimuli, RT_binned_q), by = c("stimuli", "RT_binned_q"))

# formatting to match retroact data set
mpt_data <- mutate(mpt_data, treeid = ifelse(stimuli == "ASL", 1, ifelse(stimuli == "Face", 2, ifelse(stimuli == "Object", 3, 4))))
mpt_data <- mutate(mpt_data, resp = as.factor(resp))
...
mpt_data <- as.data.frame(mpt_data)
