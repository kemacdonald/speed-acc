...
timeslice_ASL <- timeslice_setup %>%
  ...
  group_by(RT_binned_q, stimuli) %>% 
  multi_boot_standard("ss_acc") 
timeslice_ASL$RT_binned_q <- plyr::revalue(timeslice_ASL$RT_binned_q, 
                                           c("(0.0301,0.833]"= "Q1", 
                                             "(0.833,1.13]" = "Q2",
                                             "(1.13,1.5]" = "Q3", 
                                             "(1.5,2.97]" = "Q4"))

timeslice_Face <- timeslice_setup %>%
  filter(stimuli == "Face") %>%
  mutate(RT_binned = cut(RT_sec, breaks = 4),
         RT_binned_q = lsr::quantileCut(RT_sec, 4)) %>%
  group_by(RT_binned_q, stimuli, Sub.Num) %>% 
  ...
  group_by(RT_binned_q, stimuli) %>% 
  multi_boot_standard("ss_acc") 
timeslice_Face$RT_binned_q <- plyr::revalue(timeslice_Face$RT_binned_q, 
                                           c("(0.0301,0.567]"= "Q1", 
                                             "(0.567,0.867]" = "Q2",
                                             "(0.867,1.07]" = "Q3", 
                                             "(1.07,2.9]" = "Q4"))

timeslice_Obj <- timeslice_setup %>%
  filter(stimuli == "Object") %>%
  mutate(RT_binned = cut(RT_sec, breaks = 4),
         RT_binned_q = lsr::quantileCut(RT_sec, 4)) %>%
  group_by(RT_binned_q, stimuli, Sub.Num) %>% 
  ...
  group_by(RT_binned_q, stimuli) %>% 
  multi_boot_standard("ss_acc") 
timeslice_Obj$RT_binned_q <- plyr::revalue(timeslice_Obj$RT_binned_q, 
                                           c("(0.0301,0.2]"= "Q1", 
                                             "(0.2,0.433]" = "Q2",
                                             "(0.433,0.683]" = "Q3", 
                                             "(0.683,2.97]" = "Q4"))

timeslice_Bull <- timeslice_setup %>%
  filter(stimuli == "Bullseye") %>%
  mutate(RT_binned = cut(RT_sec, breaks = 4),
         RT_binned_q = lsr::quantileCut(RT_sec, 4)) %>%
  group_by(RT_binned_q, stimuli, Sub.Num) %>% 
  summarise(ss_acc = mean(correct, na.rm=T)) %>% 
  group_by(RT_binned_q, stimuli) %>% 
  multi_boot_standard("ss_acc") 
timeslice_Bull$RT_binned_q <- plyr::revalue(timeslice_Bull$RT_binned_q, 
                                           c("(0.0305,0.167]"= "Q1", 
                                             "(0.167,0.333]" = "Q2",
                                             "(0.333,0.633]" = "Q3", 
                                             "(0.633,2.57]" = "Q4"))

timeslice_setup <- rbind(timeslice_Obj, timeslice_ASL, timeslice_Face, timeslice_Bull)
