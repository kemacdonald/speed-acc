...
```{r libraries}
library(here)
source(here("code/helper_functions/libraries_v_3.6.R"))
source(here("code/helper_functions/libraries_and_functions.R"))
```
...
```{r load data}
path_demo <- "data/0a_demographics/"
path_data <- "data/2_cleaned_data/"
path_prescreen <- "data/0b_trial_information/"

read_csv_fun <- function(path, file_name) {
  df <- read_csv(paste0(path, file_name)) %>% 
    mutate_all(.funs = as.character)
}
```

Read in data and get the number of participants for each study.

```{r}
## ASL
sol_demo <- read_csv_fun(path = path_demo, file_name = "sol_demo_all.csv")
sol_df <- read_csv_fun(path_data, "sol-ichart-cleaned.csv") 
sol_n <- sol_df %>% select(Sub.Num) %>% unique() %>% summarise(ASL = n())

## Trio prescreening information 
trio_ps <- read_csv_fun(path = path_prescreen, file_name = "trio_prescreening_info.csv") 

## Trio Face
trio_face <- read_csv_fun(path = path_data, file_name = "TrioFace26n24_cleaned.csv") 
trio_n <- trio_face %>% select(Sub.Num) %>% unique() %>% summarise(Face = n())

## Trio 26
trio_26 <- read_csv_fun(path = path_data, file_name = "Trio_26_n18_cleaned.csv")
trio_26_n <- trio_26 %>% select(Sub.Num) %>% unique() %>% summarise(Trio_26 = n())

## Trio 36
trio_36 <- read_csv_fun(path = path_data, file_name =  "Trio36m_n22_cleaned.csv")
trio_36_n <- trio_36 %>% select(Sub.Num) %>% unique() %>% summarise(Trio_36 = n()) 

## Trio Adults
trio_bull_adult <- read_csv_fun(path = path_data, file_name = "TrioBullAdult_icharts_n7_cleaned.csv")
trio_bull_adult_n <- trio_bull_adult %>% select(Sub.Num) %>% unique() %>% summarise(Bull_Adult = n())

## Trio Bull
trio_bull_26 <- read_csv_fun(path = path_data, file_name = "TrioBullAA26n18_cleaned.csv")
trio_bull_26_n <- trio_bull_26 %>% select(Sub.Num) %>% unique() %>% summarise(Bull_26 = n())
```

Create a table with the number of participants in each study.
...
Add some other relevant variables.

```{r}
trio_face %<>% 
  mutate(language_modality = "English", stimuli = "Face", 
         experiment = "Trio_Face", age_code = "child", 
         hearing_status_participant = "hearing")

trio_26 %<>% 
  mutate(language_modality = "English", stimuli = "Trio", 
...
trio_bull_adult %<>%
  mutate(language_modality = "English", stimuli = "Bull", 
...
```{r}
select_vars_fun <- function(data_frame) {
  data_frame %<>%
    select(Sub.Num, Months, Tr.Num, stimuli, Months, experiment, age_code, Target.Image,
           hearing_status_participant, Prescreen.Notes, language_modality, `0`:`3000`) 
}
```

```{r}
sol_df_final <- select_vars_fun(sol_df) 
trio_face_final <- select_vars_fun(trio_face)
trio_26_final <- select_vars_fun(trio_26)
trio_36_final <- select_vars_fun(trio_36)
trio_bull_adult_final <- select_vars_fun(trio_bull_adult)
trio_bull_26_final <- select_vars_fun(trio_bull_26)
```

### Bind data frames together

```{r}
speed_acc_df_final <- bind_rows(sol_df_final, trio_face_final, 
                                trio_26_final, trio_36_final, 
                                trio_bull_adult_final, trio_bull_26_final)
```

### Clean up target image variable

Make a clean target signs variable for filtering later on.

```{r}
targets <- c("juice", "cookie", "cup", "ball", "shoe", "kitty",
             "doll", "teddy", "book", "birdy", "car", "sock", "banana")
...
speed_acc_df_final$clean_target_img <- unlist(sapply(speed_acc_df_final$Target.Image, 
                                                     function (x) make_clean_target(x, targets)))
speed_acc_df_final %<>% select(-Target.Image)
```
...
em_data <- iChart %>% 
  select(`0`:`3000`) %>% 
  mutate_all(funs(ifelse(. == "2", "0.5", .)))


iChart %<>% 
  select(Sub.Num:language_modality, clean_target_img) %>% 
  bind_cols(., em_data)
```
...
```{r}
total_trials <- 32
trials_cut_point <- total_trials * .20

ss_prescreened <- iChart %>% 
  select(Sub.Num, Tr.Num, Prescreen.Notes) %>% 
  unique() %>% 
...
ss_trials_df <- iChart %>% 
  distinct(Sub.Num, Tr.Num, experiment, age_code, language_modality) %>% 
  group_by(Sub.Num, experiment, age_code, language_modality) %>% 
  summarise(Trials = n()) 
...
ss_trials_df %>% 
  mutate(exclude_few_trials = ifelse(n_good_trials <= trials_cut_point, 
                                     "exclude", "include")) %>% 
  write_csv(here::here(filter_path, "speed-acc-trio-sub-filters.csv"))
...
```{r}
# apply function to each row in our dataset to flag trial type
...
ss <- iChart %>%
  ...
  select(Sub.Num:language_modality, trial_type, age_code, RT, clean_target_img) %>% 
...
```{r}
write.csv(iChart, file = "../../data/3_final_merged_data/speed-acc-cleaned-iChart.csv", 
          row.names = F, na = "NA")

write.csv(ss, file = "../../data/3_final_merged_data/speed-acc-ss-df.csv", 
          row.names = F, na = "NA")
```