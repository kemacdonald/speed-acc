...
explanation of the differences between young ASL- and
...
We think it is important to point out correspondence between the
reported results and our pre-registered analysis plan and predictions.
First, we predicted that the noise manipulation would cause listeners to
gather more information by lookign longer at the speaker's face (slower
RTs), and that this behavior would lead listeners to produce a higher
...
access to a rich set of multimodal cues that could support connecting
...
\begin{center}
\begin{threeparttable}
\caption{\label{tab:noise-guess-cuts}Output of the model estimating the mean proportion of shifts categorized as language-driven by the Exponentially Weighted Moving Average model for the each noise condition and age group in Experiment 1.}
\begin{tabular}{llll}
\toprule
