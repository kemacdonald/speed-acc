...
broom.mixed::tidy(trio_rt_mle) %>% 
  ...
  apa_table(placement = 'h', 
            note = "Model specification: log(RT) ~ stimuli + scale(Months) + (1 | Sub.Num) + (1 | clean_target_img)",
...
broom.mixed::tidy(trio_acc_mle) %>% 
  munge_table(mod_type =  'multilevel', exp = "e1") %>% 
  apa_table(placement = 'h',
            note = "Model specification: correct ~ stimuli + (1 | Sub.Num) + (1 | clean_target_img)",
...
broom.mixed::tidy(trio_ewma_mle) %>% 
  munge_table(mod_type =  'multilevel', exp = "e1") %>% 
  apa_table(placement = 'h',
            note = "Model specification: guess ~ stimuli + (1|subid) + (1|clean_target_img)",
...
broom.mixed::tidy(noise_rt_mle) %>% 
  ...
  apa_table(placement = 'h',
            note = "Model specification: log(rt) ~ noise_condition + age_category + (1 | target_image)",
