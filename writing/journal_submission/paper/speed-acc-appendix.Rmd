...
```{r read data and create helpers}
clean_p_vals <- function(x) {
  case_when(x < .001 ~ "p < .001",
            x < .01 ~ "p < .01", 
            TRUE ~ round(x, 2) %>% as.character(x))
}
munge_table <- function(d, mod_type =  'multilevel') {
...
$p$ values for each coefficient were computed via the Satterthwaite approximation using the `lmerTest` R package [@kuznetsova2017lmertest]. 
...
broom.mixed::tidy(trio_rt_mle) %>% 
  munge_table(mod_type =  'multilevel') %>% 
...
# Full output for Bayesian data analysis models in Experiment 1
...
# Full output for Bayesian data analysis models in Experiment 2
