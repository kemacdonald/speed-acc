...
noise_time_gvs %<>% 
  ...
  mutate(age_code = fct_relevel(age_code, "adults", "children"))
...
# put plots together
p_acc <- plot_grid(p_acc, pairwise_diffs_plot, ncol = 2, rel_widths = c(0.9, 1),
                   scale = c(1, 0.85))
```
...
pairwise_diffs_plot <- diffs_df %>% 
  ...
  geom_jitter(shape = pt_shape, 
              ...
              alpha = pt_alpha) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(ymin = ci_lower, ymax = ci_upper), 
                  data = ms_diffs,
                  shape = 15,
                  size = 4,
                  color = "grey20") +
...
p_rt <- plot_grid(p_rt, pairwise_diffs_plot, ncol = 2, rel_widths = c(0.9, 1),
                  scale = c(1, 0.85))
```
...
```{r}
noise_fst_shifts <- plot_grid(p_rt, p_acc, 
                              ncol = 1, 
...
```{r}
p_ewma_noise <- d_ewma %>%
  ggplot(aes(x = fct_rev(age_code), y = prop.responding, fill = noise_condition)) +
  geom_jitter(aes(fill = noise_condition),
              alpha = 0.3,
              shape = 21, 
              size = 3,
...
p_ewma_noise <- p_ewma_noise +
  geom_pointrange(aes(x = fct_rev(age_code), y = prop.responding,
