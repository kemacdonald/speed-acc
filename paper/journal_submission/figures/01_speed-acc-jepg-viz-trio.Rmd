...
trio_timecourse <- trio_timecourse_gvs %>% 
  ggplot(aes(x = as.numeric(Time.ms), y = mean, color = stimuli, linetype = value_cat), 
       data = .) + 
  ylim(0,1) +
  xlim(0,3) +
  geom_line(size=0.7) +
...
p_acc_trio <- ss_trio %>% 
  ggplot(aes(x = fct_rev(stimuli), y = mean_correct, fill = stimuli)) +
...
p_rt_trio <- ss_trio %>% 
  ...
  ggplot(aes(x = fct_rev(stimuli), y = m_rt, fill = stimuli)) +
  ggbeeswarm::geom_quasirandom(shape = pt_shape, 
                               width = pt_width, 
                               color = pt_color, 
                               size = pt_size,
