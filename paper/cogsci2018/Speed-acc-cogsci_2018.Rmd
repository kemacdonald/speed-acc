...
abstract: 
  "Language comprehension in grounded contexts involves integrating visual and linguistic information with the goal of establishing reference. But how do we decide to best allocate our limited attentional resources. Here, we report the results of two, novel experiments, which support the hypothesis that listeners adapt their gaze patterns to seek higher value visual information, but only when it is useful. First, we show that adults and children delayed their gaze shifts away from a language source, were more accurate with these shifts, and produced a smaller proportion of nonlanguage-driven shifts when processig language in a noisy auditory context (E1). Next, we present results showing that adults and children do not delay eye movements to wait for a speaker's gaze cue if the auditory signal is sufficient to establish reference (E2). Together, these results provide converging evidence that the dynamics of eye movements during language comprehension are quite flexible, and listeners will adapt gaze patterns to seek higher value visual information, but only when it is useful for rapid and accurate language understanding."

keywords:
  "eye movements; language processing; information-seeking; n"
