---
title: "Children seek additional visual information to support language comprehension in noisy auditory environments"
...
abstract: 
  "Skilled language comprehension in grounded contexts involves integrating visual and linguistic information. But the value of fixating on a location in the visual world depends on its usefulness for the listener's goal -- rapid language understanding. Here, we report the results of two experiments that support the hypothesis that listeners flexibly delay the timing of gaze shifts away from a language source in order to seek higher value visual information, but only when this behavior is useful. First, we show that adults (n=50) and children (n = 50, 3-5 y.o.) delayed their eye movements away from a language source while processing familiar nouns in a noisy context. Intrestingly, this decision to delay resulted in more accurate gaze shifts, fewer nonlanguage-driven or random shifts (E1). Next, we present results showing that adults (n=50) and children (n=54, 3-5 y.o.) do not delay eye movements to wait for a post-nominal social cue (eye gaze) when the auditory signal was sufficient to establish reference (E2). Together, these results provide converging evidence that the dynamics of eye movements during language comprehension are flexible, and listeners seek higher value visual information when it is useful for language understanding."

keywords:
  "eye movements; language processing; information-seeking; speech in noise; social cues"
