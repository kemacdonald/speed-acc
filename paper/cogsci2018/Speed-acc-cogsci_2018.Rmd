...
## cut point model
ms_cond_diff_cuts <- samples_ewma %>% 
  ...
  select(param, contrast, estimate)

## prop guessing model
ms_cond_diff_guessing <- samples_ewma_guess_noise %>% 
  select(sample_id, noise_beta, age_beta) %>%
  rename(age = age_beta, noise = noise_beta) %>% 
  unique() %>% 
  gather(key = contrast, value = param_est, -sample_id) %>% 
...
noise_table <- xtable::xtable(noise_table, caption = "EWMA and HDDM results for E1.")
