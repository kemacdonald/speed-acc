...
gaze_contrast <- d_models$acc_gaze %>% 
  ...
  mutate(gaze_contrast = gaze - straight_ahead) %>% 
...
# add blank level to leave room for the inset
ss_final %<>% mutate(gaze_condition = factor(gaze_condition))
levels(ss_final$gaze_condition) <- c(levels(ss_final$gaze_condition),'') 

acc_plot_e2 <- ss_final %>% 
  filter(shift_accuracy_clean == "correct") %>% 
